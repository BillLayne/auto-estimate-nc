...
const Layout: React.FC<LayoutProps> = ({ children, setStep, variant = 'auto' }) => {
      ...
      <header className="sticky top-0 z-50 bg-gradient-to-r from-brand-navy-deep via-brand-navy to-brand-navy-dk text-white shadow-lg border-b-2 border-brand-gold">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center">
          <button
            ...
            onClick={() => setStep(isHome ? AppStep.HomeLanding : AppStep.ModeSelection)}
          ...
          >
            ...
            {isHome && (
              <span className="hidden xs:inline-flex sm:inline-flex items-center bg-brand-gold/20 border border-brand-gold/50 text-brand-gold-lt text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full">
                Home Edition
              </span>
            )}
          </button>
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <div className="border-t border-white/10 mt-6 pt-5 text-center">
            ...
            <p className="text-[11px] text-blue-300/70 mt-1.5 max-w-2xl mx-auto">
              ...
              {isHome
                ? ' Final costs are set by a licensed contractor, and any claim decision is made with your insurance company.'
                : ' Final repair costs are set by a licensed body shop and your insurance adjuster.'}
            </p>
            <div className="flex items-center justify-center gap-4 mt-3">
              {!isHome && (
                <button onClick={() => setStep(AppStep.Docs)} className="text-[11px] text-blue-300/50 hover:text-blue-200 underline underline-offset-2">
                  How it works
                </button>
              )}
              <a href={isHome ? '/' : '/home/'} className="text-[11px] text-blue-300/50 hover:text-blue-200 underline underline-offset-2">
                {isHome ? 'Auto estimate tools' : 'Home repair estimates'}
              </a>
            </div>
          </div>
        </div>
