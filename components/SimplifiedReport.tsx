...
const SimplifiedReport: React.FC<SimplifiedReportProps> = ({ data, onReset }) => {
  const downloadHtml = () => {
    ...
    const fullHtml = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Explained Estimate - Bill Layne Insurance</title>
    ...
    link.href = url;
    link.download = `Bill-Layne-Estimate-Explainer-${new Date().toISOString().split('T')[0]}.html`;
    link.click();
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fadeIn">
      ...
      <div className="bg-white rounded-2xl shadow-sm border p-6 flex flex-col md:flex-row justify-between items-center gap-4 no-print">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Estimate Explanation</h2>
      ...
      <div id="simple-letterhead" className="bg-white p-8 md:p-12 shadow-lg rounded-none md:rounded-2xl text-slate-900 mx-auto max-w-[850px]">
        ...
        {/* Header */}
        <div className="border-b-4 border-emerald-600 pb-6 mb-8 flex justify-between items-start">
          <div>
            <h1 className="text-3xl font-black text-slate-800">BILL LAYNE</h1>
            <h2 className="text-sm font-bold tracking-widest text-emerald-600 uppercase">Insurance Agency Guide</h2>
          ...
          <div className="text-right">
            <div className="bg-emerald-50 text-emerald-800 px-4 py-2 rounded-lg border border-emerald-100 inline-block text-center">
              <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-600">Estimate Total</p>
        ...
        <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-8">
          <h3 className="text-emerald-700 font-bold text-sm uppercase tracking-wider mb-2 flex items-center gap-2">
            ...
            <div key={idx}>
              ...
              <div className="space-y-4">
                {section.items?.map((item, itemIdx) => (
                  <div key={itemIdx} className="bg-white border border-slate-100 p-4 rounded-lg shadow-sm hover:border-emerald-200 transition-colors">
                    ...
                    <p className="text-sm text-slate-600 italic bg-emerald-50/50 p-2 rounded">
                      "{item.explanation}"
                    </p>
                  </div>
                ))}
        ...
        <div className="mt-12 pt-8 border-t border-slate-100 text-center">
          ...
          <p className="text-[10px] text-slate-400 leading-relaxed max-w-2xl mx-auto italic">
            "I, Bill Layne, am an independent insurance agent. This document simplifies a professional estimate using AI to help you understand the repairs. It is not a promise of payment or a professional reappraisal. Always verify details with your body shop."
          </p>
        </div>
